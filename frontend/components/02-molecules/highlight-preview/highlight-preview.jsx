    ...
    useEffect(() => {
			const context = gsap.context(() => {
				if (!items || items.length === 0) return;
					...
					timeline.current.to(images, { opacity: 0, duration: 2, ease: 'inOut' }, 0);
					...
					timeline.current.to(boxes, { opacity: 0, duration: 2, ease: 'inOut' }, 0);
					timeline.current.to(boxes[counter], { opacity: 1, duration: 2, ease: 'inOut' }, 0.5);
					timeline.current.to(images[counter], { opacity: 1, duration: 2, ease: 'inOut' }, 0.5);
					timeline.current.to(status[0], { opacity: 1, duration: 2, ease: 'none ' }, 0.5);
					timeline.current.to(status[0], { right: '0%', duration: 6, ease: 'none' }, 2.5);
					timeline.current.to(status[0], { opacity: 0, duration: 2, ease: 'inOut' }, 10);
					timeline.current.to(images[counter], { opacity: 0, duration: 2, ease: 'inOut' }, 10);
					timeline.current.to(images[counter], { opacity: 0, duration: 2, ease: 'inOut' }, 10);
					timeline.current.to(boxes[counter], { opacity: 0, duration: 2, ease: 'inOut' }, 10);
			}, previewRef);
    ...
    return (
        <div className={`${ className } highlight-preview`} ref={ previewRef }>
            <div className="highlight-preview__preview preview">
                { items?.map((item) => (
                    <Picture className="preview__image" src={ item.attributes.teaser.data?.attributes.url } alt="Image" key={ item.id } width={ 2500 } height={ 2500 } quality={ 100 } priority />
                ))}
            </div>
