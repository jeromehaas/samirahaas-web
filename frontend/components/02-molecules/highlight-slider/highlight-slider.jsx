...
const HighlightSlider = ({ className, items }) => {
	...
	useEffect(() => {
		const context = gsap.context(() => {
			...
			timeline.current.to(status[counter], { opacity: 1, duration: 2, ease: 'none ' }, 0.5);
			timeline.current.to(status[counter], { right: '0%', duration: 6, ease: 'none' }, 0.5);
			timeline.current.to(status[counter], { opacity: 0, duration: 1, ease: 'inOut ' }, 8);
			timeline.current.to(images[counter], { opacity: 0, duration: 1, ease: 'inOut' }, 8);
			timeline.current.to(labels[counter], { opacity: 0.2, duration: 1, ease: 'inOut' }, 8);
			timeline.current.to(titles[counter], { opacity: 0.2, duration: 1, ease: 'inOut' }, 8);
		}, sliderRef);
