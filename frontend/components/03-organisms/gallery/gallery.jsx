...
const Gallery = ({ data }) => {
	...
	return (
		<Section className="gallery" ref={ galleryRef } key={ id }>
			...
			{ data.attributes.images ? <Impressions className="gallery__impressions" items={ data.attributes.images } /> : null }
			{ data.attributes.navigation && data.attributes.navigation?.next.data.id && data.attributes.navigation?.previous.data.id ? (
